...
- **Blip is present and performs the move** (passes the load-bearing test in
  `character.md`) — not standing beside the idea.
- **Mascot is on-model**: rounded-cube body, screen face, two dot eyes, blank
  deadpan (no brow, no mouth), one antenna. No panels/bolts/gears/UI.
- **Value matches the palette**: in light palettes the body is light with a
  structure-ink (not pure-black) screen — not a heavy dark blob.
...
- **Labels**: ≤3, short, correctly spelled, structure-ink on bare paper — never
  on a colored fill.
- **Accent discipline**: accent on the antenna + 1–2 elements only; the body and
...
- Mascot reads as a mascot/sticker/cute-cartoon, or has a mouth/eyebrows/shiny eyes → re-roll.
...
- Derived/blog palette colors off-target → eyedrop vs the target hex; re-roll or snap in post.
...
- Too cute → emphasize blank deadpan, no mouth, not a mascot.
