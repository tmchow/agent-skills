...
| [`routerbase-model-gateway`](routerbase-model-gateway/) | Use [routerbase](https://routerbase.com/) as an OpenAI-compatible model gateway — SDK base URL migration, credential boundaries, model routing, fallbacks, and live validation discipline. |
...
| [`yt-dlp-cli`](yt-dlp-cli/) | Drive [yt-dlp](https://github.com/yt-dlp/yt-dlp) for streaming-site media downloads, audio extraction, subtitle pulls, clips, and bounded playlist/channel archives, with format inspection, ffmpeg checks, output verification, and safe YouTube auth failure handling. |
